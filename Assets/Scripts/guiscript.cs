		...
		while (true) {
			...
			if (message.Length > 0) {
				...
				case cmd_mmesh:
					...
					// add to see if shows different color
					//game_obj = new GameObject();
					//myMeshFilter = new MeshFilter();
					//MeshRenderer myMeshRen = new MeshRenderer();
					Mesh meshC = CreatePlanarMesh(Nx,Nz,dNx,dNz,myfloats2);
					//Mesh meshC = game_obj.GetComponent<MeshFilter>().mesh;
					//Mesh meshC = myMeshFilter.mesh;
					...
					while (ii < vertices.Length) {
						...
						//colors[ii] =new Color(0.5F,0.5F,(vertices[ii].y)/10.0F);
						//colors[ii]=Color.green;
						ii++;
					}
					meshC.colors = colors;
					//myMeshFilter = game_obj.AddComponent<MeshFilter>();
					...
					while (nh < texture.width) {
						...
						while (nw < texture.height) {
							...
							//Color color = Color.Lerp(Color.grey, Color.green, (nh*nw)/(texture.height*texture.width));
							//texture.SetPixel(nh, nw, color);
					...
					texture.Apply();
					//myMeshRen.material;
					//myMeshRen = game_obj.AddComponent<MeshRenderer>();
					//game_obj.GetComponent<Renderer>().material.SetColor("_Color", Color.green);//an example to set the green color of whole object
				...
				case cmd_sphere:
					...
					Destroy(mySphere);
					/*
					for (int it=0;it<(int)(Nump/3);it++){
						Debug.Log(it);
						m_sphere = Instantiate (mySphere) as GameObject;
						m_sphere.transform.position = new Vector3(float.Parse(parts[it*3+2]),float.Parse(parts[it*3+2]),float.Parse(parts[it*3+2]));
						RegisterObject(m_sphere);
					...
					m_sphere.transform.position = new Vector3(float.Parse(parts[1]),float.Parse(parts[2]),float.Parse(parts[3]));
					RegisterObject(m_sphere);*/
					break;
				case cmd_sphere_mod:
					...
					parts=message.Split (',');
					//m_sphere = Instantiate (mySphere) as GameObject;
					//m_sphere=(GameObject) graphicsArray[0];
					//m_sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
					//mySphere.transform.position = new Vector3(float.Parse(parts[1]),float.Parse(parts[2]),float.Parse(parts[3]));
					//m_sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
					//Debug.Log (parts[1]);
					//Debug.Log (parts[2]);
					//Debug.Log (parts[3]);
					//var p1=float.Parse(parts[1]);
					//var p2=float.Parse(parts[2]);
					//var p3=float.Parse(parts[3]);
					//var position = Vector3(p1,p2,p3);
					//mySphere.transform.position(position);
					//new Vector3 p1 = (float.Parse(parts[1]),float.Parse(parts[2]),float.Parse(parts[3]));
					game_obj=(GameObject) graphicsArray[int.Parse (parts[1])];
					game_obj.transform.position = new Vector3(float.Parse(parts[2]),float.Parse(parts[3]),float.Parse(parts[4]));
					Debug.Log("good");
					//mySphere.SetActive(false);
					//Vector3 position = (float.Parse(parts[0]),float.Parse(parts[1]),float.Parse(parts[2]));
					//Vector3 Position = (float.Parse(parts[0]),float.Parse(parts[1]),float.Parse(parts[2]));
					//m_sphere.transform.position (position);// = (float.Parse(parts[0]),float.Parse(parts[1]),float.Parse(parts[2]));
					//RegisterObject(m_sphere);
