...
public class FileBrowser
{
    ...
    public delegate void SelectedFileIndexChangedHandler(int selection);
    public SelectedFileIndexChangedHandler OnSelectedFileChanged;
    protected int m_selectedFile
    {
        ...
        set
        {
            if (_selectedFileIndex != value)
            {
                ...
                {
                    OnSelectedFileChanged(_selectedFileIndex);
                }
            }
        }
    }
    
    protected int _selectedFileIndex = -1;
    ...
    {
        if (selection > 0 && m_files.Length > selection)
        {
            m_selectedFileName = m_files [selection];
        }
    }
    ...
    public FileBrowser(Rect screenRect, string name, FinishedCallback callback)
    {
        ...
        SwitchDirectoryNow();
        OnSelectedFileChanged += OnFileSelectionChanged;
    }
    ...
    public void OnGUI()
    {
        ...
        m_selectedFileName = GUILayout.TextField(m_selectedFileName);
        GUILayout.BeginHorizontal();
        GUILayout.FlexibleSpace();
        ...
        {
            GUI.enabled = m_selectedFile > -1 || m_selectedFileName.Length > 0;
        } else
        {
            if (SelectionPattern == null)
            {
        ...
        if (GUILayout.Button("Select", GUILayout.Width(50)))
        {
            if (BrowserType == FileBrowserType.File)
            {
                m_callback(Path.Combine(m_currentDirectory, m_selectedFileName));//m_files [m_selectedFile]));
